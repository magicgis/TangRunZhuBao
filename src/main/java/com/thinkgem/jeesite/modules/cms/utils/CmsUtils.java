...
public class CmsUtils {
	...
	public static List<Category> getMainNavList(String siteId){
		...
		if (mainNavList == null){//mainNavList的值是从这里设定（首次在这里设定，以后每次在这里取）
    ...
    public static String getUrlDynamic(Article article) {
        ...
        //2017-11-22  .append("-0")
        str.append("/view-").append(article.getCategory().getId()).append("-").append(article.getId()).append("-0").append(Global.getUrlSuffix());
        return str.toString();
    }
	 ...
	 * @param siteId 站点编号
	 */
	public static List<Product> getProductList(String siteId){
		@SuppressWarnings("unchecked")
		...
		if (productList == null){//productList的值是从这里设定（首次在这里设定，以后每次在这里取）
			...
			Page<Product> page = new Page<Product>(1, -1);
